...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Sorry!</b> The group already exists');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Sorry!</b> The "group level" has already been entered in!');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          //sucess
          $session->msg('s',"The group has been created");
          redirect('add_group.php', false);
        } else {
          //failed
          $session->msg('d',' Failed to create group');
          redirect('add_group.php', false);
        }
   } else {
